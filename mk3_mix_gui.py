...
# ==========================================
# Guarda a última configuração enviada com sucesso para o Windows
current_config = {
    "applied": False,
...
# Dicionário para forçar a atualização visual dos botões à distância
update_funcs = {}
...
if getattr(sys, 'frozen', False):
    # Se estiver a correr como um ficheiro .exe compilado
    PASTA_ATUAL = os.path.dirname(sys.executable)
else:
    # Se estiver a correr no PyCharm / Terminal
    PASTA_ATUAL = os.path.dirname(os.path.abspath(__file__))
...
def obter_processos_brutos():
    ...
    try:
        resultado = subprocess.run("tasklist /V /FO CSV /NH", shell=True, capture_output=True, text=True,
                                   errors='ignore', creationflags=NO_WINDOW)
...
def verificar_alteracoes(*args):
    """Verifica se a UI atual tem diferenças para a última configuração aplicada."""
    mudou = (
    ...
    # Try/Except para evitar o erro de inicialização (quando o botão ainda não foi desenhado)
    try:
        if mudou:
            # Acorda o botão e fica Vermelho (Pronto a aplicar)
            btn_aplicar.configure(state="normal", text="▶ APPLY MIX", fg_color=COR_MUTE, text_color="#FFFFFF")
        else:
            # Adormece o botão e fica Verde
            btn_aplicar.configure(state="disabled", text="✅ MIX ATIVO", fg_color=COR_LINE1,
                                  text_color_disabled="#000000")
    except NameError:
        pass  # Ignora silenciosamente enquanto a interface ainda está a arrancar


def atualizar_displays_ao_vivo():
    """Lê APENAS o que foi aplicado com sucesso e expande nos ecrãs."""
    l1_apps = []
    l2_apps = []
...
def aplicar_configuracoes():
    ...
    if st1 != "MUTE" and st1 == st2:
        ...
        # Reverte visualmente para o último estado seguro
    ...
    if all([s1, s2, s3, s4]):
        # Guarda a nova configuração como sendo a ativa
        ...
        verificar_alteracoes()  # Fica verde e bloqueia!
    else:
...
# Janela ajustada para caber os textos dos ecrãs de forma dinâmica
...
ctk.CTkButton(frame_launch, text="🌐 CH1", command=lambda: abrir_programa("chrome1"), **style_launch).grid(row=0,
                                                                                                          column=0,
                                                                                                          padx=5,
                                                                                                          pady=5)
ctk.CTkButton(frame_launch, text="🌐 CH2", command=lambda: abrir_programa("chrome2"), **style_launch).grid(row=0,
                                                                                                          column=1,
                                                                                                          padx=5,
                                                                                                          pady=5)
ctk.CTkButton(frame_launch, text="▶ VLC", command=lambda: abrir_programa("vlc"), **style_launch).grid(row=0, column=2,
                                                                                                      padx=5, pady=5)
ctk.CTkButton(frame_launch, text="⚙️ SETUP", text_color="#FFFFFF", fg_color=COR_MUTE, hover_color="#B0123C",
              font=("Segoe UI", 12, "bold"), height=38, width=120, command=abrir_configuracoes).grid(row=0, column=3,
                                                                                                     padx=5, pady=5)
...
ctk.CTkLabel(frame_ch1, text="■ CH 1: YouTube (chrome1)", font=("Impact", 18), text_color="#FFFFFF").grid(row=0,
                                                                                                          column=0,
                                                                                                          padx=15,
                                                                                                          pady=25,
                                                                                                          sticky="w")
...
ctk.CTkLabel(frame_ch2, text="■ CH 2: YouTube (chrome2)", font=("Impact", 18), text_color="#FFFFFF").grid(row=0,
                                                                                                          column=0,
                                                                                                          padx=15,
                                                                                                          pady=25,
                                                                                                          sticky="w")
...
ctk.CTkLabel(frame_ch3, text="■ CH 3: Media Players (VLC, Spotify)", font=("Impact", 18), text_color="#FFFFFF").grid(
    row=0, column=0, columnspan=2, padx=15, pady=(15, 5), sticky="w")
combo_app3 = ctk.CTkComboBox(frame_ch3, width=260, height=35, font=("Segoe UI", 12),
                             command=lambda _: verificar_alteracoes())
...
ctk.CTkLabel(frame_ch4, text="■ CH 4: Extras / Karaoke", font=("Impact", 18), text_color="#FFFFFF").grid(row=0,
                                                                                                         column=0,
                                                                                                         columnspan=2,
                                                                                                         padx=15,
                                                                                                         pady=(15, 5),
                                                                                                         sticky="w")
combo_app4 = ctk.CTkComboBox(frame_ch4, width=260, height=35, font=("Segoe UI", 12),
                             command=lambda _: verificar_alteracoes())
...
# Monitor LINE 1
frame_disp_l1 = ctk.CTkFrame(frame_displays, fg_color="#1A1A1A", border_width=2, border_color=COR_LINE1,
                             corner_radius=10)
...
# A fonte do display foi ajustada para caberem 3/4 canais perfeitamente na mesma caixa
lbl_display_line1 = ctk.CTkLabel(frame_disp_l1, text="---", font=("Segoe UI", 12, "bold"), text_color="#FFFFFF",
                                 justify="center")
lbl_display_line1.pack(pady=(5, 15), padx=10)

# Monitor LINE 2
frame_disp_l2 = ctk.CTkFrame(frame_displays, fg_color="#1A1A1A", border_width=2, border_color=COR_LINE2,
                             corner_radius=10)
...
lbl_display_line2 = ctk.CTkLabel(frame_disp_l2, text="---", font=("Segoe UI", 12, "bold"), text_color="#FFFFFF",
                                 justify="center")
...
btn_atualizar = ctk.CTkButton(frame_botoes, text="⟳ RESCAN", font=("Segoe UI", 14, "bold"), fg_color="#333333",
                              hover_color="#555555", text_color="#FFFFFF", width=140, height=45,
...
btn_aplicar = ctk.CTkButton(frame_botoes, text="▶ APPLY MIX", font=("Segoe UI", 16, "bold"), fg_color=COR_MUTE,
                            hover_color="#B0123C", text_color="#FFFFFF", width=250, height=45,
                            command=aplicar_configuracoes)
